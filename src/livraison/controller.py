...
def read_mode_livraison(db: Session = Depends(get_db)):
    return get_mode_livraison(db)

@router.get("/transporteur", response_model=list[TransPorteurResponse])
def read_transporteur(db: Session = Depends(get_db)):
    return get_transporteur(db)
